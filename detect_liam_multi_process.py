...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        # # print('serial_data type:', type(serial_data))
        # decoded_info = serial_data.split(',')
        # tof_dis_threshold = 150
        # tof1, tof2, tof3 = int(decoded_info[0]), int(decoded_info[1]), int(decoded_info[2])
        # if tof1 < 15:
        #     tof1 = 8190
        # if tof2 < 15:
        #     tof2 = 8190
        # if tof3 < 15:
        #     tof3 = 8190
        # switch1, switch2, switch3, switch4 = int(decoded_info[4]), int(decoded_info[5]), int(decoded_info[6]), int(decoded_info[7]) 
        ...
        #         print(elem)
        # if tof1 < tof_dis_threshold or tof2 < tof_dis_threshold or tof3 < tof_dis_threshold:
        ...
        # print('pred nums:{}'.format(len(pred)))
        # print('pred[0]:', pred[0])
        # exit()
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    # print('xyxy[0].cpu().item():', xyxy[0].cpu().item())
                    # print('xyxy[2].cpu().item():', xyxy[2].cpu().item())
                    # print('xyxy[0].cpu().item() type:', type(xyxy[0].cpu().item()))
                    # exit()
            ...
            # print('robot_center_points:', robot_center_points)
            # print('trunk_center_points:', trunk_center_points)
            # print('robot_corners:', robot_corners)
            # print('trunk_corners:', trunk_corners)
            ...
            #         line_length = line_length * scale_
            #         str_2_draw = str(round(line_length, 2)) + 'm'
            #         cv2.putText(im0, str_2_draw, str_center, cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 128), 2, cv2.LINE_AA)
            ...
            #         # res = rectangle_invation(rb_corner, trunk_corner)
            #         # print('rectangle_invation res:', res)
            ...
            # print('=========== dis info ===========')
            # for elem in dis_list_per_loop:
            #     print('human pos: ({}, {})  dis: {}'.format(elem[0][0], elem[0][1], elem[1]))
            # print('=========== dis info ===========')
...
def parse_opt():
    ...
    parser.add_argument('--weights',
                        ...
                        default='/home/liam/Weights/dec_real_scenario/yolov5n/best.pt',
...
def main(opt):
    ...
    p1 = mp.Process(target=run, args=('0', queue1))
    vars(opt)['source'] = 2
    print_args(vars(opt))
    p2 = mp.Process(target=run, args=('1', queue2))
